...
namespace basecross {
	...
	constexpr int MAXNUM = 10;//!割る数字
	constexpr float HELFSIZE = 0.5f;//!ポリゴンサイズ
	constexpr float STARTSCALEZ = 1.0f;//!テクスチャのサイズz
	constexpr float STARTPOSZ = 0.0f;//!テクスチャのポジションz
	constexpr float PICECPOS = 1.0f;//!桁ごとの位置
	constexpr float RIGHUV = 0.1f;//!右側のuv頂点
	...
	void MyTime::OnCreate() {
		...
		ptrTrans->SetRotation(Vec3(0.0f));                          //!回転の設定
	...
	void MyTime::OnUpdate() {
		...
		for (UINT i = m_NumberOfDigits; i > 0; i--) {
			...
			num = num / (base / MAXNUM);     //!残った数字がその桁に入る
